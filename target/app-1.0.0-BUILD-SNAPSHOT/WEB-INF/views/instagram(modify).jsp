...
<head>
    ...
    <script src="<c:url value='/js/instagram(modify).js'/>"></script>

    <link rel="stylesheet" href="<c:url value='/css/instagram(h_f).css'/>">
    <link rel="stylesheet" href="<c:url value='/css/instagram(modify).css'/>">
...
<div id="wrap" class="contents_area">
    <div class="haeder">
        ...
        <div class="hampan">
            ...
            <div class="ham_menu">
                ...
                <div class="ham_desc"><a href="">프로필</a></div>
            </div>
            <div class="ham_menu">
                <div class="ham_img">
    ...
    <form action="<c:url value='/myPage/save'/>" method="post" enctype="multipart/form-data" class="main_bot" onsubmit="return register_check()" >
        <div class="main">

            <div class="main_top">
                <div class="main_imgbox" id="imgArea">
                    <c:choose>
                        <c:when test="${userInfo.user_photo == null}">
                            <!-- 프로필 이미지가 없으면 기본 이미지 -->
                            <img src="./img/로고2.jpg" class="main_img">
                        </c:when>
                        <c:otherwise>
                            <img src="<c:url value='./img/${userInfo.getUser_photo()}'/>" class="main_img">
                        </c:otherwise>
                    </c:choose>
                </div>
                <div class="filebox">
                    <input id="uploadFile" class="upload-name" value="프로필 업로드" placeholder="이미지 업로드">
                    <label for="file" >이미지 업로드</label>
                    <input type="file" id="file" name="filename1">
                </div>
            </div>
            <div class="error" id="error"></div>

            <input type="text" placeholder="아이디(닉네임)" class="id" id="id" name="user_id" value="${userInfo.getUser_id()}" readonly>
            <input type="text" placeholder="성명" class="name" name="user_name" id="name" value="${userInfo.getUser_name()}">
            <input type="text" placeholder="전화번호(숫자만 입력)" class="pone" name="ph_num" id="pone" value="${userInfo.getPh_num()}">
            <select name="gender" id="" class="gender" >
                <option value="1" ${userInfo.getGender() == 1 ? "selected" : " "}>남성</option>
                <option value="2" ${userInfo.getGender() == 2 ? "selected" : " "}>여성</option>
            </select>
            <div class="main_descbox">
                <textarea name="user_desc" class="main_desc" placeholder="내용 입력">${userInfo.getUser_desc()}</textarea>
                <div class="main_descbox_bot">
                    <span>0</span>/1000
                </div>
            </div>
